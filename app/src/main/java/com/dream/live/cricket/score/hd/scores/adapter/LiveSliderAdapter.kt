...
class LiveSliderAdapter(
    val context: Context,
    private val navigateData: NavigateData,
    val source: String
) :
    ListAdapter<LiveScoresModel, LiveSliderAdapter.LiveSliderAdapterViewHolder>(
        LiveSliderAdapterDiffUtilCallback
    ) {

    class LiveSliderAdapterViewHolder(private var binding: RecentitemBinding,private val context: Context) :
        RecyclerView.ViewHolder(binding.root) {
        fun bindScoreData(liveScoresModel: LiveScoresModel) {
            ...
            if (data.start_time != null && data.end_time != null) {
                ...
                val valueShow = Cons.convertDateAndTime(data.start_time!!)
                dateAndTime(valueShow, data.start_time!!, data)
            }
        }

        private fun dateAndTime(channelDate: String?, startTime: Long, dataValue: LiveScoresModel) {
                ...
                if (hours >= 0) {
                    if (min >= 0) {
                        if (hourOfMatch > currentParseHour) {
                            ...
                            binding?.startTime?.text = context.getString(R.string.startTime, hours, min)
                        } else {
                        ...
                        if (hourOfMatch > currentParseHour) {
                            ///means match will start later...
                            binding?.startTime?.text = context.getString(R.string.startTime, hours, x)
                        } else {
                            binding?.startTime?.text = context.getString(R.string.startedTime, hours, x)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LiveSliderAdapterViewHolder {
        val binding:RecentitemBinding  = DataBindingUtil.inflate(LayoutInflater.from(context),
            R.layout.recentitem,parent,false)
